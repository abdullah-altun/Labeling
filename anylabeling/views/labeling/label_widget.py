...
import glob
from drive.Admin import AdminControl
...
class LabelingWidget(LabelDialog):
    ...
    def __init__(
        ...
        # self.deneme_dock = QtWidgets.QDockWidget(self.tr("denme"),self)
        # self.deneme_dock.setObjectName("Deneme")
        # self.deneme_dock.setWidget(self.label_list)
        # self.shape_dock.setStyleSheet(dock_title_style)
        ...
        self.text = QLabel()
        self.text.setText("Dosya Durumu.....")
        self.text.setAlignment(Qt.AlignRight)
        self.text.setMaximumSize(150,60)
        text_den = QLabel()
        button_full = QPushButton("ALL")
        button_full.setMaximumSize(120,60)
        button1 = QPushButton("Revize")
        button1.setMaximumSize(120,60)
        button2 = QPushButton("Kontrol edilmemiş")
        button2.setMaximumSize(120,60)
        button3 = QPushButton("Doğru Görüntü")
        button3.setMaximumSize(120,60)

        button_full.clicked.connect(self.ALL)
        button1.clicked.connect(self.Revize)
        button2.clicked.connect(self.Control)
        button3.clicked.connect(self.True_image)

        alt_layout = QHBoxLayout()
        alt_layout.addWidget(self.label_instruction)
        alt_layout.addWidget(text_den)
        alt_layout.addWidget(button_full)
        alt_layout.addWidget(button1)
        alt_layout.addWidget(button2)
        alt_layout.addWidget(button3)
        alt_layout.addWidget(self.text)
        central_layout.addLayout(alt_layout)
    ...
    def _path(self,path):
        parts = []
        while True:
            path, last_part = os.path.split(path)
            if last_part:
    ...
    def ALL(self):
        self.UserType = self.drive_screen.userType
        if self.UserType == "admin":
            self.import_image_folder(images_list=True)
        else:
            self.text.setText("Yetkiniz Yok!!!")

    def Revize(self):
        if not(self.drive_screen == None):
            self.UserType = self.drive_screen.userType
        if self.UserType == "admin":
            self.text.setText("Görüntü revize edilecek")
            if self.filename == None:
                self.text.setText("Görüntü seçiniz!!!!")
            else:
                paths = self._path(self.filename)
                self.Admin.json_control(paths,"Revize edilecek")
                self.drive_screen.update_upload()
        else:
            self.text.setText("Yetkiniz Yok!!!")
    ...
    def Control(self):
        if not(self.drive_screen == None):
            self.UserType = self.drive_screen.userType
        if self.UserType == "admin":
            self.text.setText("Kontrol Klasörü")
            if self.filename == None:
                self.text.setText("Görüntü seçiniz!!!!")
            else:
                paths = self._path(self.filename)
                self.Admin.json_control(paths,"Kontrol edilecek")
                self.drive_screen.update_upload()
        else:
            self.text.setText("Yetkiniz Yok!!!")
    ...
    def True_image(self):
        if not(self.drive_screen == None):
            self.UserType = self.drive_screen.userType
        if self.UserType == "admin":
            self.text.setText("Görüntü Doğru")
            if self.filename == None:
                self.text.setText("Görüntü seçiniz!!!!")
            else:
                paths = self._path(self.filename)
                self.Admin.json_control(paths,"Kontrol edildi")
                self.drive_screen.update_upload()
        else:
            self.text.setText("Yetkiniz Yok!!!")
    ...
    def set_dirty(self):
        ...
        label_file = label_file.replace("images", "mask")
            # if self.output_dir:
    ...
    def drive_file(self):
        if not self.drive_screen:
            self.drive_screen = main.DriveScreen()
        else:
            self.drive_screen.update_upload()
        self.drive_screen.show()
    ...
    def scan_all_USER_ID(self,UserIds):
        images_list = []
        if not(self.drive_screen == None):
            self.UserType = self.drive_screen.userType
        for UserId in UserIds:
            if self.UserType == "user":
                images_list = self.Admin.image_file(UserId,images_list)
            else: 
                images_list = self.Admin.admin_image_file(UserId,images_list)
            # images_list.extend(glob.glob(f"drive/DataSet/{UserId}/images/**"))
        return images_list

    def import_image_folder(self, dirpath=None, pattern=None, load=True,images_list=None):
        self.actions.open_next_image.setEnabled(True)
        self.actions.open_prev_image.setEnabled(True)
        ...
        if images_list == None:
            images_list = self.scan_all_USER_ID(UserIds)
        else:
            images_list = []
            for UserId in UserIds:
                images_list.extend(glob.glob(f"drive/DataSet/{UserId}/images/**"))        
